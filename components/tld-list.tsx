...
export function TldList({ initialLimit = 20 }: { initialLimit?: number }) {
  ...
  const { visible: visibleTlds } = useStaggeredReveal(tlds, { stepMs: 40 })
  ...
  return (
    <div>
      ...
      <div className="mode-panel rounded border border-border bg-background">
        {visibleTlds.map((tld, index) => {
          const renewalKnown = Number.isFinite(tld.renewalPrice) && tld.renewalPrice > 0
          return (
            <div
              key={tld.extension}
              className={
                "reveal-item flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 px-4 py-3 font-mono text-sm" +
